...
const EventSinglePage = ({ events, slug }) => {
	...
	const { date, image, tickets, time, name, location, description, organizer, price } = attributes;
	return (
		<Layout title={name}>
				...
				<div className='container'>
					...
					<div className='row'>
						<div className='col-lg-8'>
							<img className='w-100 object-fit-cover' src={`${API_URL}${image.data.attributes.url}`} alt='' />
							...
							{events?.slice(0, 3).map((evt) => (
								<div key={evt.id} className='col-md-6 col-lg-4 mb-4 mb-lg-0'>
									<div className='upcoming-events__item'>
										<div className='image'>
											<img className='img-fluid' src={`${API_URL}${evt.attributes?.image?.data?.attributes.url}`} alt={evt.attributes.image.data.attributes.name} />
										...
										<div className='upcoming-events__item__info'>
											...
											<div className='d-flex align-items-center justify-content-between mb-2'>
												...
												<div className='d-flex align-items-center gap-2'>
													<ImTicket /> <span>{evt.attributes.tickets} remaining</span>
												</div>
											</div>
